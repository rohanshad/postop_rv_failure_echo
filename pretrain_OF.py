'''
Script: pretrain_OF.py
Dependency environment: tf_gpu

Script to pretrain a ResNet152 network on optical flow from videos derived from the echo dataset from Oayang et al.
...
class DataGeneratorEchonet(Sequence):
	...
	* y = array of labels for each movie clip, turned into one hot vectors
	...
	'''
	def __next__(self, debug=False):
		if self.n >= self.max:
			self.n = 0
		if debug:
			result = self.__getitem__(self.n, debug=True)
		else:
...
if __name__ == '__main__':
	...
	train_direc = os.path.join(root_direc, 'train_norm_OF') #define where training movies are
	test_direc = os.path.join(root_direc, 'val_norm_OF') #define where testing movies are
	...
	csv_direc = os.path.join(ckpt_direc, 'model_csv') #define where to store csv file with model training history will be stored
	...
	#history = model.fit_generator(generator=gen, validation_data=test_gen, epochs=num_epochs, callbacks=[model_checkpoint, epoch_logger], initial_epoch = init_epoch, use_multiprocessing=True, workers=num_cpu)
